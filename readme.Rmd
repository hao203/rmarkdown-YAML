...
1. Scalars, or variables, are defined using a colon and a **space**.A dictionary is represented in a simple `key: value` form (the colon must be followed by a `space`)
...
In YAML, spaces(indent) are used to indicate nesting. When we want to specify the output function `pdf_document(toc = TRUE)`, we need to nest it under the `output` field. We also need to nest `toc` under pdf_document so that it gets passed to that function correctly.
